...
2. For local use on Windows with Python installed, double-click tools/start-local.bat. It opens http://localhost:8080. Keep that window open. Alternatively run python tools/serve-local.py.
...
4. Open index.html through the server, enter Sat800, and choose AI setup → LM Studio. Default address: http://localhost:1234/v1. If server authentication is enabled, enter its token under the optional authentication section. Gemini keys are never sent to LM Studio.
